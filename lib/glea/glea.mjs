...
export default class GLea {
  ...
  createTexture(textureIndex = 0, params = { 
    textureWrapS: 'clampToEdge', textureWrapT: 'clampToEdge',
    textureMinFilter: 'nearest', textureMagFilter: 'nearest'
  }) {
    const scream = (str = "") => (/^[A-Z0-9_]+$/.test(str) ? str : str.replace(/([A-Z])/g, '_$1').toUpperCase());
    const { gl } = this;
  ...
  uniV(name, data) {
    const { gl, program } = this;
    const loc = gl.getUniformLocation(program, name);
    gl["uniform" + data.length + "fv"](loc, (data instanceof Array || data instanceof Float32Array) ? data : new Float32Array(data));
    return loc;
  }
